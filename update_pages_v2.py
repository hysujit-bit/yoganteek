...
# Header HTML template (from Yoganteek_ad_enquiry.html)
HEADER_HTML = '''<!-- Shared Header -->
...
<style>
  #main-nav { transition: padding 0.3s ease, box-shadow 0.3s ease; }
  #main-nav.scrolled { padding-top: 0.5rem; padding-bottom: 0.5rem; box-shadow: 0 4px 24px rgba(0,0,0,0.09); }
  ...
  .hamburger span { display: block; width: 22px; height: 2px; background: #2C2C2C; border-radius: 2px; transition: transform 0.3s ease, opacity 0.2s ease; transform-origin: center; }
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
      <div class="md:col-span-2">
        <a href="index.html" class="inline-block mb-4">
          <img src="./assets/logo.svg" alt="Yoganteek Logo" class="h-10 w-auto">
        </a>
...
# Update pages (skip Yoganteek_ad_enquiry.html as it's the reference)
...
pages = [
    ...
    ('Yoganteek_Corporate_Landing_Page.html', 'Corporate'),
    ('blog.html', 'Blog'),
    ('blog-be-concentric.html', 'Blog Post'),
    ('blog-stress-management.html', 'Blog Post'),
    ('blog-yoga-for-kids.html', 'Blog Post'),
    ('blog-yoga-for-women.html', 'Blog Post'),
]

for filename, page_name in pages:
    filepath = os.path.join(frontend_dir, filename)
    if os.path.exists(filepath):
        update_page(filepath, page_name)
