...
# Header HTML (Tailwind-based, from Yoganteek_ad_enquiry.html)
...
(function(){
  ...
  btn.addEventListener('click',function(){btn.classList.toggle('open');menu.classList.toggle('open');document.body.style.overflow=menu.classList.contains('open')?'hidden':''});
  var p=window.location.pathname.split('/').pop()||'index.html';
  document.querySelectorAll('.nav-link,.mobile-link').forEach(function(l){
    var d=l.getAttribute('data-page');
    if(p.indexOf(d)!==-1||(p===''&&d==='index'))l.classList.add('active');
  });
  document.querySelectorAll('.mobile-link').forEach(function(l){l.addEventListener('click',function(){btn.classList.remove('open');menu.classList.remove('open');document.body.style.overflow=''})});
})();
</script>'''
...
def process_file(filepath):
    ...
    while i < len(lines):
        ...
        if '</head>' in stripped and not added_tailwind:
        ...
        if '<!-- TOP BAR -->' in stripped or '<!-- top bar -->' in stripped.lower():
            ...
            while i < len(lines) and '<!-- NAV -->' not in lines[i] and '<nav' not in lines[i].lower():
                i += 1
            # If we found NAV comment, skip it too
            if i < len(lines) and '<!-- NAV -->' in lines[i]:
                i += 1
            # Skip until after </nav>
        ...
        # Skip old nav if no TOP BAR comment
        if '<nav' in stripped.lower() and not added_header and ('nav' in stripped.lower() or 'class="nav' in stripped):
            # Check if this looks like the old nav
            if 'class="nav' in stripped or 'id="nav' in stripped:
                while i < len(lines) and '</nav>' not in lines[i]:
                    i += 1
                i += 1  # Skip the </nav> line
                if not added_header:
        ...
        # Skip old footer
        if ('<!-- FOOTER -->' in stripped or '<footer' in stripped.lower()) and not added_footer:
            # Check if it's the old style footer
    ...
    # If we didn't add header/footer (file structure was different), add at end
    if not added_header:
        # Find <body> and add after it
